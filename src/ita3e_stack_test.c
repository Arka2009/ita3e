...
int ita3e_linked_stack_test() {
	ita3e_item_linked_stack_t lstack;
	int push_status, pop_status;
	ita3e_item_t x;
	ita3e_item_linked_stack_init(&lstack);
	...
	while(i <= num_insertions) {
		...
		push_status = ita3e_item_linked_stack_push(&lstack,tmp);
		...
		ita3e_item_linked_stack_print(&lstack);
		printf("\n");
		i++;
	}
	...
	while(i <= num_deletions) {
		pop_status = ita3e_item_linked_stack_pop(&lstack,&x);
